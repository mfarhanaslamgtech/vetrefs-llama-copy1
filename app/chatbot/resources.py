...
class ClinicalAnalysisRequest(BaseModel):
    case_text: str = Field(..., min_length=10, max_length=50000)

    @field_validator("case_text", mode="before")
    @classmethod
    def normalize_case_text(cls, value):
...
def analyze_clinical_case(
    request_data: ClinicalAnalysisRequest,
    chatbot: Chatbot = Depends(get_chatbot),
):
    """Analyze a transcript or SOAP note as ranked differentials."""
    try:
        return chatbot.analyze_case(request_data.case_text)
    except BackendServiceError:
        raise
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except Exception as exc:
        logging.error("Clinical case analysis failed: %s", str(exc), exc_info=True)
        _raise_model_http_error(exc)
        raise HTTPException(status_code=500, detail="Clinical case analysis failed.") from exc
